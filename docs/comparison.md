...
| Capability toggles | Organization Admin enables **MCP server**, **Agentic capabilities**, **Omni documentation search**, **Single shot query generation**, **Content access** in AI Hub — server-side, org-wide | `OMNI_TOOL_MODULES` picks which tool modules are registered (all of them by default) — client-side, per session, and a context/blast-radius knob rather than a permission: it cannot grant anything the key lacks, and the API surface itself is whatever the key can do |
